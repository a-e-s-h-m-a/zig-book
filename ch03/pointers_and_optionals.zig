...
// A simple struct representing a sensor device with a numeric reading.
const Sensor = struct {
    reading: i32,
};

/// Print a sensor's reading value to debug output.
...
pub fn main() !void {
    ...
    // ?*T representing a pointer that may or may not hold a valid address
    ...
    // Many-item pointers ([*]T) allow unchecked indexing withour length tracking
    const many: [*]i32 = view.ptr;

    // Modify the underlying array through the many-item pointer
    // No bound checking is performed at thid point
    many[2] = 42;

    std.debug.print("slice view len={}\n", .{view.len});
    // Verify that modification through many-item pointer affeced the original array
