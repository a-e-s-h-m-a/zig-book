...
pub fn main() !void {
    ...
    const message = requireDebugSafetyV2() catch |err| {
        try stdout.print("Error: running in release mode: {}\n", .{err});
        return err;
    };
...
fn requireDebugSafetyV2() error{ReleaseOnly}![]const u8 {
    if (@import("builtin").mode == .ReleaseFast or
        @import("builtin").mode == .ReleaseSmall or
        @import("builtin").mode == .ReleaseSafe)
    {
        return error.ReleaseOnly;
    }
    return "Debug safety checks are enabled";
}
